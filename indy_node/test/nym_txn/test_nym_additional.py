...
def test_send_same_nyms_only_first_gets_written(looper, sdk_pool_handle, sdk_wallet):
    wh, _ = sdk_wallet
    ...
    # request 1
    _, did1 = sdk_add_new_nym(
        looper, sdk_pool_handle, sdk_wallet, dest=did, verkey=verkey
    )
...
def test_get_nym_without_adding_it(
...
@pytest.fixture(scope="module")
def verkey_removed_from_existing_did(
    looper,
    sdk_pool_handle,
    sdk_wallet_steward,
    verkey_added_to_nym,
...
def testNewverkey_added_to_nym(
    be, do, philCli, abbrevIdr, verkeyRemovedFromExistingDID
):
    pass
