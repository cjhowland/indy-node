...
brew tap-new $USER/local-libsodium
brew extract --version=1.0.12 libsodium $USER/local-libsodium
brew install libsodium@1.0.12
...
brew extract --version=5.8.8 rocksdb $USER/local-rocksdb
brew install rocksdb@5.8.8
...
pushd /tmp
git clone https://github.com/domwoe/indy-sdk.git
pushd indy-sdk/libindy
git checkout -b feature/did_indy
cargo build --release
